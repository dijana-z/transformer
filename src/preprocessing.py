...
def load_vocabulary(infile, min_occurrences=50):
    ...
    with codecs.open(infile, 'r', 'utf-8') as f:
        ...
        words = [row['TOKEN'] for row in reader if row['FREQUENCY'] >= min_occurrences]
...
def create_data(source_sentences, target_sentences, en_vocab_file, de_vocab_file, maxlen=15):
    ...
    x_data, y_data, source, target = [], [], [], []

    for sentence, target in zip(source_sentences, target_sentences):
        x = [en_wti.get(word, 1) for word in sentence.split()] + [en_wti['</S>']]
    ...
    return x_data, y_data, source, target
...
def create_train_sentences(infile):
    ...
    return [regex.sub('[^\s\p{Latin}\']', '', line) for line in codecs.open(infile, 'r', 'utf-8').read().split('\n') if
...
def create_datasets(flags):
    ...
    """
    # Load data
    x_train, y_train, *_ = load_data(flags.train_inputs, flags.train_labels, flags.en_vocab_file, flags.de_vocab_file,
                                     create_train_sentences)
    x_test, y_test, *_ = load_data(flags.test_inputs, flags.test_labels, flags.en_vocab_file, flags.de_vocab_file,
